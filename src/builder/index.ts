...
type Props<T extends Constructor> = {
  [K in keyof Properties<T>]: (value: Properties<T>[K]) => PropsOfInstance<Omit<Properties<T>, K>, T>
} & BuilderBuild<T, T>;

/**
  * builder for data classes
  **/
export function builder<T extends Constructor<any>>(buildee: T, ...args: unknown[]): Props<T> {
